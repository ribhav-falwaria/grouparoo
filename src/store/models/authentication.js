...
const authentication = {
  ...
  effects: (dispatch) => ({
    async signInUser({ email, password }, rootState) {
      crashlytics().log(
        ErrorUtil.createLog(
          " signInUser method starts here",
          { rootState, email, password },
          "signInUser()",
          "authentication.js"
        )
      );
      try {
        ...
        const customerDetails =
          await apiService.appApi.customer.getCustomerByUserId(customer.$id);
        const loanApplications =
          await apiService.appApi.loanApplication.getAllLoanApplications(
            customerDetails.$id
          );
        Promise.all([
    ...
    async registerOrUpdateUser({ formData, password }, rootState) {
      crashlytics().log(
        ErrorUtil.createLog(
          " registerOrUpdateUser method starts here",
          { formData, password, rootState },
      ...
      let customer;
      try {
        customer = await apiService.appApi.auth.login(
          formData.email,
        ...
        try {
          ...
          return dispatch.authentication.loginFailed({
            isFirstTime,
            accountExists,
          });
        }
