...
const getUpiChoices = async () => {
  crashlytics().log(
    ErrorUtil.createLog(
      "getUpiChoices method starts here",
      undefined,
      "getUpiChoices()",
      "Payments.js"
    )
...
const generatePaymentToken = async (loanId, amount, mode, id) => {
  crashlytics().log(
    ErrorUtil.createLog(
      "generatePaymentToken method starts here",
      { loanId, amount, mode, id },
      "generatePaymentToken()",
      "Payments.js"
    )
  );
  const env = config.PAYMENT_ENV;
  const executionId = await apiService.appApi.cashfree.token.execute(
    loanId,
    amount,
    env
  );
...
const Payments = ({ navigation, route }) => {
  ...
  const handlePaymentCallback = async (result) => {
    ...
    if (result.signature && result.signature.length > 0) {
      const executionId =
        await apiService.appApi.cashfree.signatureVerify.execute(orderResult);
      verified = await apiService.appApi.cashfree.signatureVerify.get(
        executionId
      );
    }
    if (verified) {
  ...
  const useGeneratePaymentToken = useRequest(generatePaymentToken, {
    ...
    onSuccess: (result) => {
      ...
      const paramData = {
        ...
        color2: translations.formatString(translations["payments.for"], {
          loanId,
        }),
      };
      if (mode === METHOD_UPI) {
        ...
        RNPgReactNativeSdk.startPaymentUPI(
          paramData,
          env,
  ...
  const renderPaymentItem = ({ item }) => {
    crashlytics().log(
      ErrorUtil.createLog(
        "renderPaymentItem method starts here",
        { item },
        "renderPaymentItem()",
        "Payments.js"
      )
    ...
    const accessoryLeft = (props) => {
      crashlytics().log(
        ErrorUtil.createLog(
          "accessoryLeft method starts here",
          { props },
          "accessoryLeft()",
          "Payments.js"
        )
      ...
      if (item.id !== "debitcard" && item.id !== "netbanking") {
        return (
          <View style={styles.iconContainer}>
            {useGeneratePaymentToken.loading ? (
              <Spinner size="tiny" />
            ) : (
              <Image
                {...props}
    ...
    return (
      <ListItem
        title={
          <Text category="p1" status="info" style={{ marginLeft: 32 }}>
            {item.displayName}
          </Text>
        }
  ...
  const renderUpiHeader = () => {
    crashlytics().log(
      ErrorUtil.createLog(
        "renderUpiHeader method starts here",
        undefined,
        "renderUpiHeader()",
        "Payments.js"
      )
  ...
  const rendeDebitCardrHeader = () => {
    crashlytics().log(
      ErrorUtil.createLog(
        "rendeDebitCardrHeader method starts here",
        undefined,
        "rendeDebitCardrHeader()",
        "Payments.js"
      )
    );
    return (
  ...
  const { transactionComplete, success, tryAgain } = npStatus;
  ...
  return (
    <View style={styles.container}>
      <ScreenTitle
        title={title}
        description={translations["payment.description"]}
      />
      ...
      {isSuccess && <PaymentSuccessView />}
      {isFailure && <PaymentFailureView onCancel={onCancel} />}
      {isTryAgain && <PaymentRetryView onRetry={onRetry} onCancel={onCancel} />}
    </View>
  );
};
