...
const ImageUploadComponent = ({
  ...
  crashlytics().log(
    ErrorUtil.createLog(
      "ImageUploadComponent method starts here",
      {
        isUploadDone,
        onFileChange,
        useFrontCamera,
  ...
  return (
    <>
      {(isUndefined(uris) || uris.length === 0 || !isUploadDone) && (
        <TouchableOpacity onPress={() => refRBSheet.current.open()}>
          <Card style={themeStyle.cardContainer} status={hasError && "danger"}>
            {!loading && <Text category="s1">{selectText}</Text>}
            {loading && <Spinner />}
          </Card>
        </TouchableOpacity>
      )}
      {!isUndefined(uris) && uris.length > 0 && (
        <View>
          <Card
            style={themeStyle.cardContainerV1}
...
const DisplayImageCard = ({ uri, onDeleteUpload }) => {
  const themeStyle = useStyleSheet(styles);
  return (
