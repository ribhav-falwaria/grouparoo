...
const generateOtp = async (appId, aadharNumber) => {
  ...
  try {
    const res = await DataService.postData(
      resource.constants.kyc.getUrlToGenerateOTP,
      {
        appId: appId,
...
const verifyOtp = async (appId, otp, shareCode, panData) => {
  ...
  try {
    const res = await DataService.postData(
      resource.constants.kyc.getUrlToFetchKyc,
      {
        aadhaar_otp: otp,
    ...
    if (
      msg === "CANNOT_VALIDATE_AADHAR_OTP" ||
      msg === "NAME_MATCH_FAILED" ||
      msg === "IDENTITY_MATCH_FAILED" ||
      msg === "CAN_NOT_REACH__AADHAR_PAN_MATCH_SERVER"
    ) {
      throw err;
...
const matchKycData = async (panData, kycData) => {
  ...
  try {
    const res = await DataService.postData(
      resource.constants.kyc.getUrlForIdMatch,
      {
        panData,
        kycData,
      }
