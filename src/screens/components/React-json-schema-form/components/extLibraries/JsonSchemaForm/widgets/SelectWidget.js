...
const SelectWidget = ({
  ...
  const [selectedValue,setSelectedValue] = useState(value ? getLabelFromValue(enumOptions,value) : "");
    ...
    >
      {enumOptions.map((option, i) => {
        ...
        const label = radioLabelMapping
          ...
          : option.label
        return <SelectItem key={`selopt-${i}`} title={label} disabled={itemDisabled} />
      })}
    </Select>
  )
}
